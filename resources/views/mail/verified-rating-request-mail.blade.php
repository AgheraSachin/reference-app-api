...
<table width="600" cellpadding="0" cellspacing="0" align="center" style="background:#fff">
    ...
    <tr>
        ...
        <td>
            ...
            <p style="font-size: 20px; color: #4A5566; margin-bottom: 10px;">Hi {{explode("@",$sent_to_email)[0]}},
                <br><br/>Your
                colleague <b>{{$user['first_name']}} {{$user['last_name']}}</b> has requested a work reference from you!
                Please click below to complete your feedback and send it
                to {{$user['first_name']}} {{$user['last_name']}}. Have a wonderful day!</p>
            <p style="margin-bottom: 30px; font-size: 18px; color: #4A5566;">* Please note that in order for us to
                verify your identity as a reference, you must connect with your LinkedIn account.</p>
            <a href="https://reference.app/submit-reference?token={{$token}}" target="_blank"
               style="max-width: 290px;width: 100%;display: block;color: #fff;height: 56px;background: #0052E2;border-radius: 6px;font-weight: bold;text-align: center;vertical-align: middle;text-decoration: none;line-height: 56px">Connect
                with LinkedIn to Start</a>
        </td>
        <td></td>
    </tr>
    <tr>
