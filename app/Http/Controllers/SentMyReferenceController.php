...
class SentMyReferenceController extends Controller
{
    ...
    public function verifyAccessCode(Request $request)
    {
        ...
        if (SendVerifiedReference::where(['access_code' => $request->get('access_code'), 'access_token' => $request->get('access_token')])->exists()) {
            ...
            foreach ($send_videos as $key => $val) {
                $video_data = VerifiedRatingRequest::with('user')->where('to_user_id', $val['reference_id'])->first()->toArray();
    ...
    public function sendNotification(Request $request)
    {
        ...
        foreach ($request->get('to_user_id') as $key => $val) {
            ...
            Notification::create(['to_user_id' => $val, 'notification' => '<b>' . $request->get('email') . '</b> has ask you to setup a personal call with your reference <b>' . $user->first_name . ' ' . $user->last_name . "</b>.Don't forgot to organize this call as soon as possible"]);
        }
